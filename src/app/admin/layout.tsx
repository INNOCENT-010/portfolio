...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#fff' }}>
      <header style={{ padding: '14px 32px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: '24px' }}>
          {['projects', 'skills', 'experience', 'profile', 'messages'].map(section => (
            <a key={section} href={`/admin/${section}`}
              style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'rgba(255,255,255,0.35)', textDecoration: 'none', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
              {section}
            </a>
          ))}
        </div>
